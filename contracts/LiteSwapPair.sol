...
import './ILiteswapPair.sol';
import './LiteswapERC20.sol';
import './Math.sol';
import './UQ112x112.sol';
import './IERC20.sol';
import './ILiteswapFactory.sol';
import './ILiteswapCallee.sol';

contract LiteSwapPair {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) Mint(feeTo, liquidity);
                }
            }
        } else if (_kLast != 0) {
            kLast = 0;
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) ILiteswapCallee(to).pancakeCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
